...
""")
    ...
    @Query("SELECT * FROM vault_table WHERE (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL)) AND is_deleted=0 AND (owner_id = :ownerId OR (:ownerId IS NULL AND owner_id IS NULL)) ORDER BY created_at DESC")
    fun fetchDocuments(filterId:String?, ownerId: String?): Flow<List<VaultEntity>>

    @Query("SELECT * FROM vault_table WHERE (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL)) AND is_deleted=0 AND (owner_id = :ownerId OR (:ownerId IS NULL AND owner_id IS NULL)) AND doc_type =:docType ORDER BY created_at DESC")
    ...
    @Query("UPDATE vault_table SET doc_type = :docType,  doc_date = :docDate, is_edited = 1 WHERE local_id = :localId AND (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL))")
    ...
    @Query("UPDATE vault_table SET filter_id = :filterId, auto_tags = :autoTags, doc_date = :documentDate ,tags = :tags, is_analyzing = :isAnalysing, hash_id = :hasId, cta = :cta WHERE local_id = :localId AND doc_id = :docId")
    ...
    @Query("UPDATE vault_table SET is_deleted=1 WHERE (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL)) AND local_id=:localId")
    suspend fun deleteDocument(filterId: String?, localId: String)

    @Query("SELECT * FROM vault_table WHERE doc_id IS NULL AND (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL)) AND is_deleted = 0 AND (owner_id = :ownerId OR (:ownerId IS NULL AND owner_id IS NULL))")
    suspend fun getUnSyncedDocuments(filterId: String?, ownerId: String?): List<VaultEntity>

    @Query("SELECT * FROM vault_table WHERE (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL)) AND local_id=:localId")
    ...
    @Query("SELECT * FROM vault_table WHERE (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL)) AND is_deleted=1 AND (owner_id = :ownerId OR (:ownerId IS NULL AND owner_id IS NULL))")
    suspend fun getDeletedDocuments(filterId: String?, ownerId: String?): List<VaultEntity>

    @Query("SELECT * FROM vault_table WHERE (filter_id = :filterId OR (:filterId IS NULL AND filter_id IS NULL)) AND is_edited=1 AND (owner_id = :ownerId OR (:ownerId IS NULL AND owner_id IS NULL))")
    suspend fun getEditedDocuments(filterId: String?, ownerId: String?): List<VaultEntity>

    @Query("SELECT local_id FROM vault_table WHERE filter_id=:filterId AND source=:source")
    suspend fun getLocalIdBySource(filterId:String?, source: Int): List<String>
    ...
    fun fetchDocumentsWithoutFilePath(ownerId: String?, filterId: String?): List<VaultEntity>
